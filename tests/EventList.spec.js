...
describe('EventList.vue', () => {
    ...
    it('sets correct aria attributes for accessibility', () => {
        ...
        const event = wrapper.find('.single-event');
        expect(event.find('.date span').attributes('aria-label')).toBe('Event date');
        expect(event.find('.time').attributes('aria-label')).toBe('Event time');
        expect(event.find('.type-badge').attributes('aria-label')).toBe('Event category');
        expect(event.find('.btn').attributes('aria-label')).toBe('Buy Ticket for Summer Music Festival');
    });
